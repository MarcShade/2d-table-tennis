...
class Paddle:
    ...
    def compute_dist_from_ball(self, ball_pos):
        ...
        return abs(self.a*ball_pos[0]+self.b-ball_pos[1])/sqrt(self.a * self.a + 1)
    
    def compute_center_dist(self, ball_pos):
        return sqrt((self.position[0] - ball_pos[0]) * (self.position[0] - ball_pos[0]) + (self.position[1] - ball_pos[1]) * (self.position[1] - ball_pos[1]))